...
-- Best effort: each command only works if its binary is on the system,
-- otherwise the split terminal simply shows the error.
vim.keymap.set("n", "<leader>r", function()
  vim.cmd("write")

  local ft = vim.bo.filetype
  local file = vim.fn.expand("%:p")
  local bin = "/tmp/nvim-run-" .. vim.fn.getpid()

  local commands = {
    cpp = { "bash", "-c", "g++ '" .. file .. "' -o '" .. bin .. "' && '" .. bin .. "'" },
    python = { "python3", file },
